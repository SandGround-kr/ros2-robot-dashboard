...
| 기능 | 설명 |
|------|------|
| 토픽 자동 감지 | rosbridge 연결 후 3초마다 새 토픽·노드 자동 탐지, 사라지면 자동 제거 |
| 모듈별 그룹화 | MID360, ZED Mini, Ublox, Velodyne 등 25개 모듈 자동 분류 |
| 토픽 구독·뷰어 | 구독 시 플로팅 팝업 오픈, 다중 구독 시 그리드 분할 표시 |
...
| 자동 업데이트 | 대시보드에서 버튼 클릭 한 번으로 최신 버전 적용 및 자동 재시작 |
...
### 2026-06-03
- **자동 업데이트 기능 추가** — 대시보드 헤더에서 버튼 클릭 한 번으로 최신 버전 적용
  - 페이지 로드 시 GitHub `version.json`과 자동 비교, 신버전 있으면 헤더에 버튼 표시
  - 클릭 시 현재/최신 버전 및 변경 이력(커밋 목록) 모달 표시
  - 적용 버튼 → `git pull` + `npm install` 실행 → 서버 자동 재시작 → 페이지 자동 새로고침
- **토픽 뷰어 플로팅 팝업으로 교체** — 하단 고정 뷰어 제거, 화면 우하단에 드래그·리사이즈 가능한 팝업으로 변경
  - 구독 토픽 수에 따라 자동 그리드 분할 (1개 → 1열 / 2~4개 → 2열 / 5개+ → 3열)
  - 팝업 닫아도 구독 유지, 툴바 "⊞ 뷰어" 버튼으로 재오픈
- **토픽·노드 사라짐 자동 감지** — 노드 종료 시 3초 내 UI에서 자동 제거, 구독 중이던 토픽은 자동 해제
